...
BEGIN
	...
	where (DIPENDENTE.Nome = inNome or DIPENDENTE.Nome is null)
		and (DIPENDENTE.Cognome = inCognome or DIPENDENTE.Cognome is null);
END$$
...
DELIMITER $$
USE `directory_aziendale`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `trovaUfficiConPostazioneVuota`(in nomeMansione varchar(45), in nomeSettore varchar(45))
BEGIN
...
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE `directory_aziendale`.`PIANO` TO 'dipendenteSettoreSpazi';
GRANT INSERT, SELECT, DELETE, UPDATE ON TABLE `directory_aziendale`.`UFFICIO_FISICO` TO 'dipendenteSettoreSpazi';
...
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE `directory_aziendale`.`TRASFERITO_A` TO 'dipendenteSettoreSpazi';
GRANT SELECT ON TABLE `directory_aziendale`.`DIPENDENTE` TO 'dipendenteSettoreSpazi';
...
GRANT DELETE, INSERT, SELECT ON TABLE `directory_aziendale`.`MANSIONE` TO 'dipendenteSettoreAmministrativo';
...
GRANT DELETE, INSERT, SELECT, UPDATE ON TABLE `directory_aziendale`.`DA_TRASFERIRE_A` TO 'dipendenteSettoreAmministrativo';
