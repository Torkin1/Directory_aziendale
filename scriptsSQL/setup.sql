...
CREATE TABLE IF NOT EXISTS `directory_aziendale`.`UFFICIO_FISICO` (
  ...
  `NumPiano` INT(10) UNSIGNED NOT NULL,
  `IndirizzoEdificio` VARCHAR(45) NOT NULL,
...
DELIMITER $$
USE `directory_aziendale`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `elencaTrasferimentiDipendente`(in inCFDipendente char(16))
BEGIN
	select *
    from TRASFERITO_A
...
DELIMITER $$
USE `directory_aziendale`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ricercaDipendente`(in nome varchar(45), in cognome varchar(45))
BEGIN
...
GRANT INSERT, SELECT, DELETE ON TABLE `directory_aziendale`.`UFFICIO_FISICO` TO 'dipendenteSettoreSpazi';
